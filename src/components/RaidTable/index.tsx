...
const RaidTable: Component<RaidTableProps> = (props) => {
  return (
    <table class="w-full">
      <thead class="h-8">
        <tr>
          <th class="border-[3px] border-white bg-zinc-800 px-8 py-2 text-white">관문</th>
          <th class="border-[3px] border-white bg-zinc-800 text-white">재료</th>
          <th class="border-[3px] border-white bg-zinc-800 text-white">골드</th>
        </tr>
      </thead>
      <tbody>
        <For each={Object.entries(RAID_DATA[props.raid]?.[props.level] ?? {})}>
          {([gate, data]) => (
            <tr>
              <td class="border-[3px] border-white bg-zinc-100 px-5 py-3 text-center">{gate}</td>
              ...
              <td class="border-[3px] border-white bg-zinc-100 px-5 py-3 text-left leading-4">
                {data.재화}골드 <br />
